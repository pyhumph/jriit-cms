...
const PAGE_NAME = 'electronics-telecommunications'
const COURSE_NAME = 'Electronics & Telecommunications'
...
export default function ElectronicsTelecommunicationsPage() {
        ...
        if (missingComponents.length > 0 && session?.user?.id) {
          const createPromises = missingComponents.map(async (componentName) => {
            ...
            if (componentName === 'header') {
              defaultData.title = 'Electronics &'
              defaultData.subtitle = 'Telecommunications'
              defaultData.content = 'Connect. Transmit. Communicate.'
            } else if (componentName === 'career-skills') {
              defaultData.title = 'TELECOM-READY SKILLS? CHECK.'
              defaultData.content = "In Electronics & Telecommunications at JRIIT, you'll gain expertise in electronic circuits, communication systems, signal processing, and network technologies. Our program covers analog and digital electronics, wireless communications, fiber optics, and network protocols. These are the skills that make JRIIT telecom graduates leaders in communication technology.\n\nWhy? We emphasize practical electronics projects and communication system design. You'll receive detailed feedback on your technical skills and system implementations, giving you a comprehensive understanding of your professional development.\n\nDiscover our telecommunications programs below and unlock the skills to connect the world."
          ...
          setComponents(allComponents.sort((a, b) => a.order - b.order))
        } else {
          setComponents(existingComponents.sort((a, b) => a.order - b.order))
        }
      }
    } catch (error) {
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-blue-800 mb-3">
              The programs list is managed separately through the{' '}
              <a href="/dashboard/programs" className="underline font-medium hover:text-blue-600">
                Programs Management
              </a>{' '}
              page. Make sure programs are assigned to the "{COURSE_NAME}" department.
            </p>
          </div>
        </div>
      </div>
